...
```bash
python -m resemble_enhance.denoiser.train --yaml config/denoiser.yaml --device cuda:0 --run_id <wandb_run_id> runs/denoiser
```
...
```bash
python -m resemble_enhance.enhancer.train --yaml config/enhancer_stage1.yaml --device cuda:2 --run_id <wandb_run_id> runs/enhancer_stage1
```
